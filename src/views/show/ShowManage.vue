  ...
  <div>
    <el-form ref="searchForm" :inline="true" :model="searchMap" class="demo-form-inline queryForm">
      ...
      <el-form-item prop="content">
        <el-input v-model="searchMap.content" placeholder="内容"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="list" height="600px" border style="width: 100%">
      ...
      <el-table-column prop="comment_num" label="评论" width="80px">
        <template slot-scope="scope">
          <router-link tag="a" :to="'/show-manage/comment/' + scope.row.id" class="table-link">详情</router-link>
         </template>
      </el-table-column>
...
<script>
import {showSearch, showUpdate, getshowById, showDeleteById} from '@/apis/show'
export default {
  data() {
  ...
  methods: {
    ...
    fetchData() {
      showSearch(this.currentPage - 1, this.pageSize, this.searchMap)
        .then(res => {
          if (res) {
    ...
    updateData(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          showUpdate(this.pojo).then(resp => {

<template>
  <div>
    <el-page-header @back="goBack" class="go-back" content="评论详情页">
    </el-page-header>
    ...
    <el-table :data="list" height="600px" border style="width: 100%">
      <el-table-column type="index" label="序号" width="68px"></el-table-column>
...
<script>
import {commentSearch,getCommentById,commentUpdate,commentDeleteById} from '@/apis/show'
export default {
  data() {
  ...
  methods: {
    ...
    fetchData() {
      ...
      commentSearch(this.currentPage - 1, this.pageSize, this.searchMap, id)
        .then(res => {
          if (res) {
    ...
    handleDelete(id) {
      this.$confirm("确认删除这条记录吗？", "提示", {
        ...
        .then(() => {
          commentDeleteById({id: id}).then(res => {
            const resData = res.data;
            this.$message({
              message: resData.msg,
    ...
    updateData(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          commentUpdate(this.pojo).then(resp => {
